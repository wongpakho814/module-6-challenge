...
function displayWeather(weather, city) {
    ...
    let count = 0; // Counts how many days of forecast we have rendered
    for (i; i < weather.list.length; i++) {
        if (weather.list[i].dt_txt.includes("12:00:00")) {
            renderForecastWeather(i, weather);
            count++;
        }
    }
    // If only 4 "12:00:00" items have been found and rendered, render the final element in the list as the fifth day
    if (count === 4) {
        renderForecastWeather(39, weather);
    }
}
...
// Creates and renders the HTML for the next 5 days' forecasted weather
function renderForecastWeather(i, weather) {
    let temp = weather.list[i].main.temp;
    let wind = weather.list[i].wind.speed;
    let humid = weather.list[i].main.humidity;
